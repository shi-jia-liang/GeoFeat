...
class GeoFeatLoss(nn.Module):
    ...
    # def compute_normals_loss(self, normals1, normals2, DA_normals1, DA_normals2, coco_batch_size):
        loss=[]
        
        # import pdb;pdb.set_trace()
        
        # only MegaDepth image need depth-normal
        # Use negative indexing to safely select the last N (megadepth_batch_size) elements
        # This works regardless of whether COCO data is present or what coco_batch_size is set to.
        if megadepth_batch_size > 0:
            normals1=normals1[-megadepth_batch_size:,...]
            normals2=normals2[-megadepth_batch_size:,...]
